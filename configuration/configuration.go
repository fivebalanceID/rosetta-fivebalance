...
const (
	...
	// Mainnet is the Fivebalance Mainnet.
	Mainnet string = "MAINNET"

	// Testnet is Fivebalance Testnet3.
	Testnet string = "TESTNET"

	// mainnetConfigPath is the path of the Fivebalance
	// configuration file for mainnet.
	mainnetConfigPath = "/app/fivebalance-mainnet.conf"

	// testnetConfigPath is the path of the Fivebalance
	// configuration file for testnet.
	testnetConfigPath = "/app/fivebalance-testnet.conf"
	...
	// https://github.com/fivebalance/fivebalance/blob/ad2952d17a2af419a04256b10b53c7377f826a27/src/validation.h#L84
	pruneDepth = int64(10000) //nolint

	// min prune height (on mainnet):
	// https://github.com/fivebalance/fivebalance/blob/62d137ac3b701aae36c1aa3aa93a83fd6357fde6/src/chainparams.cpp#L102
...
func LoadConfiguration(baseDirectory string) (*Configuration, error) {
	...
	case Online:
		...
		config.FivebalancedPath = path.Join(baseDirectory, fivebalancedPath)
		if err := ensurePathExists(config.FivebalancedPath); err != nil {
			return nil, fmt.Errorf("%w: unable to create fivebalanced path", err)
		}
	case Offline:
		config.Mode = Offline
	case "":
	...
	case Mainnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: fivebalance.Blockchain,
			Network:    fivebalance.MainnetNetwork,
		}
		config.GenesisBlockIdentifier = fivebalance.MainnetGenesisBlockIdentifier
		config.Params = fivebalance.MainnetParams
		config.Currency = fivebalance.MainnetCurrency
	...
	case Testnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: fivebalance.Blockchain,
			Network:    fivebalance.TestnetNetwork,
		}
		config.GenesisBlockIdentifier = fivebalance.TestnetGenesisBlockIdentifier
		config.Params = fivebalance.TestnetParams
		config.Currency = fivebalance.TestnetCurrency
