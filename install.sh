...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for coinbase/rosetta-fivebalance 
...
execute() {
  ...
  docker tag "rosetta-fivebalance:${TAG}" "rosetta-fivebalance:latest"
  log_info "loaded rosetta-fivebalance:${TAG} and tagged as rosetta-fivebalance:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-fivebalance
FORMAT=tar.gz
OWNER=coinbase
REPO="rosetta-fivebalance"
PREFIX="$OWNER/$REPO"
