...
var (
	...
	// ErrNotReady is returned when fivebalanced is not
	// yet ready to serve queries.
	ErrNotReady = &types.Error{
		Code:      2, //nolint
		Message:   "Fivebalanced is not ready",
		Retriable: true,
	}

	// ErrFivebalanced is returned when fivebalanced
	// errors on a request.
	ErrFivebalanced = &types.Error{
		Code:    3, //nolint
		Message: "Fivebalanced error",
	}
	...
	// fivebalance.ScriptPubKeys to construct a transaction.
	...
	// fivebalance.ScriptPubKey cannot be parsed during construction.
