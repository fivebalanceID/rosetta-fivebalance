...
type Client interface {
	...
	GetRawBlock(context.Context, *types.PartialBlockIdentifier) (*fivebalance.Block, []string, error)
	ParseBlock(
		context.Context,
		*fivebalance.Block,
...
func Initialize(
	...
	asserter, err := asserter.NewClientWithOptions(
		...
		fivebalance.OperationTypes,
		fivebalance.OperationStatuses,
		services.Errors,
		nil,
	)
	if err != nil {
...
// waitForNode returns once fivebalanced is ready to serve
// block queries.
...
// Sync attempts to index Fivebalance blocks using
// the fivebalance.Client until stopped.
...
// Prune attempts to prune blocks in fivebalanced every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			// Must meet pruning conditions in fivebalance core
			// Source:
			// https://github.com/fivebalance/fivebalance/blob/a63a26f042134fa80356860c109edb25ac567552/src/rpc/blockchain.cpp#L953-L960
			...
			logger.Infow("attempting to prune fivebalanced", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune fivebalanced",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned fivebalanced", "prune height", prunedHeight)
			}
		}
	}
}
...
func (i *Indexer) findCoin(
	...
	for ctx.Err() == nil {
		...
		// multiple listeners)
		transactionHash := fivebalance.TransactionHash(coinIdentifier)
...
func (i *Indexer) findCoins(
	...
	for _, coinIdentifier := range remainingCoins {
		// Wait on Channel
		txHash := fivebalance.TransactionHash(coinIdentifier)
...
func (i *Indexer) Block(
	...
	// get raw block
	var btcBlock *fivebalance.Block
	var coins []string
	var err error
...
func (i *Indexer) GetScriptPubKeys(
	ctx context.Context,
	coins []*types.Coin,
) ([]*fivebalance.ScriptPubKey, error) {
	databaseTransaction := i.database.ReadTransaction(ctx)
	defer databaseTransaction.Discard(ctx)

	scripts := make([]*fivebalance.ScriptPubKey, len(coins))
	for j, coin := range coins {
		coinIdentifier := coin.CoinIdentifier
		transactionHash, networkIndex, err := fivebalance.ParseCoinIdentifier(coinIdentifier)
		if err != nil {
		...
		for _, op := range transaction.Operations {
			if op.Type != fivebalance.OutputOpType {
				continue
			}
			...
			var opMetadata fivebalance.OperationMetadata
