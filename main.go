...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*fivebalance.Client, *indexer.Indexer, error) {
	client := fivebalance.NewClient(
		fivebalance.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return fivebalance.StartFivebalanced(ctx, cfg.ConfigPath, g)
	})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-fivebalance halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-fivebalance sync failed", "error", err)
	}
}
