...
GO_PACKAGES=./services/... ./indexer/... ./fivebalance/... ./configuration/...
...
build-release:
	...
	docker build -t rosetta-fivebalance:$(version) .;
	docker save rosetta-fivebalance:$(version) | gzip > rosetta-fivebalance-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/fivebalance-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 5555:5555 rosetta-fivebalance:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-fivebalance:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/fivebalance-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18333:18333 rosetta-fivebalance:latest
